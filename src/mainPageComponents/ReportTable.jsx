...
test(meeting)
{
		...
		console.log(t.toString().replace(".","/"))
            ...
            return (
				...
				.map((meeting) => (
                    ...
                    <tr>
						...
						<td>{meeting.duration= this.calculateMeetingDuration()}</td>
	...
	calculateMeetingDuration = () =>{
		...
		let travelT
		travelT=Number(this.travelTime )*2
	const startHours = parseInt(this.startTime.substring(0,2) , 10 )
	const startMinutes =parseInt(this.startTime.substring(3,5) , 10 )
    const endHours = parseInt(this.endTime.substring(0,2) , 10 )
	const endMinutes = parseInt(this.endTime.substring(3,5) , 10 )
...
calculateTotalTime= () =>{
	let addHours
	addHours	=this.state.sumOfMinutes/60
	let addMinutes
	addMinutes	=this.state.sumOfMinutes%60
	this.state.sumOfHours +=addHours
	if((this.state.sumOfMinutes+addMinutes)>=60){
		this.state.sumOfMinutes+=(this.state.sumOfMinutes+addMinutes)%60
		this.state.sumOfHours+=1
	}else{
